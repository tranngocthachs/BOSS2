...
public class PerformAddMarkerPage extends Page {

	public PerformAddMarkerPage() throws PageLoadException {
	...
	protected void handlePost(PageContext pageContext, Template template,
		...
		try {
			...
			if (staffInterfaceQueriesDao.isStaffModuleAccessAllowed(pageContext.getSession().getPersonBinding().getId(), assignment.getModuleId())) {
				assignmentDao.addMarkerAssociation(assignmentId, personId);
			} else {
				f.abortTransaction();
		...
		templateContext.put("nextPage", pageContext.getPageUrl(StaffPageFactory.SITE_NAME, StaffPageFactory.MARKERS_PAGE));
